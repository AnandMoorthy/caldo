...
export default function Header({ user, onSignInWithGoogle, onSignOut, onExportJSON, onImportJSON, onOpenHelp, onOpenSearch, onOpenPomodoro, currentStreak = 0, deleteAllTasksEnabled = false, onDeleteAllTasks, currentView = 'month', onChangeView, activeTab = 'tasks', onChangeTab, pomodoroEnabled = true, showTabs = true, showSearch = true }) {
  ...
  useEffect(() => {
    function onDocClick(e) {
      if (!profileMenuRef.current && !mobileProfileMenuRef.current) return;
      if (profileMenuRef.current && profileMenuRef.current.contains(e.target)) return;
      if (mobileProfileMenuRef.current && mobileProfileMenuRef.current.contains(e.target)) return;
    ...
    <header className={`sticky top-0 z-40 pb-2 sm:pb-0 header-top-spacing ${isScrolled ? 'bg-white/60 dark:bg-slate-900/70 backdrop-blur-md' : 'bg-transparent'} transition-colors flex-shrink-0 w-full`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center justify-between sm:hidden">
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 px-1.5 py-1">
            {showSearch && (
              <button 
                onClick={onOpenSearch} 
                className="text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 p-2 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
            ...
            <motion.div
              ...
              className={`inline-flex items-center gap-1.5 h-7 px-2 rounded-full text-xs bg-orange-50 text-orange-700 border border-orange-200 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-900/50 ${streakGlow ? 'ring-2 ring-orange-300/50 dark:ring-orange-400/40 shadow-[0_0_0_6px_rgba(251,146,60,0.15)] transition-shadow' : ''}`}
            ...
            <div className="relative" ref={mobileProfileMenuRef}>
              <button
                ...
                className="inline-flex items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-slate-400/50 dark:focus:ring-slate-500/40 min-w-[44px] min-h-[44px]"
                aria-haspopup="menu"
              ...
              {showProfileMenu && (
                <div className="absolute right-0 sm:right-0 -right-2 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-lg rounded-lg p-2 z-50">
