      ...
      <motion.form initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onSubmit={onSubmit} className="relative bg-white rounded-2xl shadow-lg p-6 z-50 w-full max-w-lg">
        ...
        <div className="grid gap-3">
          ...
          <div className="flex items-center gap-2">
            <label className={`px-3 py-1 rounded cursor-pointer ${form.priority === "low" ? "bg-green-100" : "bg-slate-50"}`}>
              <input type="radio" name="p" checked={form.priority === "low"} onChange={() => setForm((f) => ({ ...f, priority: "low" }))} className="hidden" /> Low
            </label>
            <label className={`px-3 py-1 rounded cursor-pointer ${form.priority === "medium" ? "bg-amber-100" : "bg-slate-50"}`}>
              <input type="radio" name="p" checked={form.priority === "medium"} onChange={() => setForm((f) => ({ ...f, priority: "medium" }))} className="hidden" /> Medium
            </label>
            <label className={`px-3 py-1 rounded cursor-pointer ${form.priority === "high" ? "bg-red-100" : "bg-slate-50"}`}>
