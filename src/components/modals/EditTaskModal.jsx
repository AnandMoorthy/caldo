...
export default function EditTaskModal({ open, editForm, setEditForm, onSubmit, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
      ...
      <motion.form initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onSubmit={onSubmit} className="relative bg-white rounded-2xl shadow-lg p-6 z-50 w-full max-w-lg">
        ...
        <div className="flex items-center gap-2">
          <label className={`px-3 py-1 rounded cursor-pointer ${editForm.priority === "low" ? "bg-green-100" : "bg-slate-50"}`}>
            <input type="radio" name="ep" checked={editForm.priority === "low"} onChange={() => setEditForm((f) => ({ ...f, priority: "low" }))} className="hidden" /> Low
          </label>
          <label className={`px-3 py-1 rounded cursor-pointer ${editForm.priority === "medium" ? "bg-amber-100" : "bg-slate-50"}`}>
          ...
          <label className={`px-3 py-1 rounded cursor-pointer ${editForm.priority === "high" ? "bg-red-100" : "bg-slate-50"}`}>
          ...
          <div className="flex items-center justify-end gap-2">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded bg-slate-50">
              Cancel
            </button>
